...
import com.fred.QrScan.camera.BitmapLuminanceSource;
import com.fred.QrScan.camera.CameraManager;
import com.fred.QrScan.decode.DecodeFormatManager;
...
public class CodeUtils {
    ...
    public static void analyzeBitmap(String path, AnalyzeCallback analyzeCallback) {
        ...
        // 解码的参数
        // 可以解析的编码类型
        List<BarcodeFormat> decodeFormats = new Vector<>();
        // 这里设置可扫描的类型，我这里选择了都支持
        decodeFormats.addAll(DecodeFormatManager.ONE_D_FORMATS);
        decodeFormats.addAll(DecodeFormatManager.QR_CODE_FORMATS);
        decodeFormats.addAll(DecodeFormatManager.DATA_MATRIX_FORMATS);

        Map<DecodeHintType, Object> hints = new Hashtable<>(2);
            ...
            for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                    if (x >= offsetX && x < offsetX + scaleWidth && y >= offsetY && y < offsetY + scaleHeight) {
                        ...
                        if (pixel == 0) {
                            pixel = bitMatrix.get(x, y) ? 0xff000000 : 0xffffffff;
                        }
                        pixels[y * w + x] = pixel;
                    } else {
